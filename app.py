...
def _load_schemas_from_file(path: str):
    """"Dynamically load facts/rule schemas from a Python file.
...
try:
    ...
    raise RuntimeError(
        ...
        "Set SCHEMAS_FILE envioment override this path."
    )
    # ------------------------
# Helpers
# ------------------------
...
def load_json(filename: str):
    """Load JSON data from ./data/<filename>"""
...
def error_payload(e: ValidationError, filelabel: str):
    """Produce rich error payload for Schema validation."""
    return {
        "file": filelabel,
        "error": e.message,
        "path": list(e.schema_path),
...
try:
    ...
    print(f"ERROR: Data validation error on startup: {e.message}")
    exit(1)

PARENT = taxonomy.get("parent", {})
FACT_VALUES = {f["id"]: bool(f["value"]) for f in facts}
...
def expand_observations(obs_conf: dict) -> dict:
    """Propagate observations confidence up the taxonomy."""
    if not PARENT:
        return obs_conf
...
def validate_payloads():
    ...
    if "facts" in payload:
        try:
            validate(instance=payload["facts"], schema=facts_array_schema)
        except ValidationError as e:
            errors.append(error_payload(e, "facts"))

    if "rules" in payload:
        try:
            validate(instance=payload["rules"], schema=rules_array_schema)
        except ValidationError as e:
...
def infer():
    """
    Inference endpoint supporting both GET and POST.

    GET:http://127.0.0.1:5000/infer?facts=f1,f2&weights=f1:1.0,f2:0.8&useTrueFacts=false
    POST: JSON body with facts, weigets, useTrueFacts
    """
    ...
    if request.method == "GET":
        ...
        if weights_param:
            for pair in weights_param.split(","):
                if ":" in pair:
                    fid, wstr = pair.split(":", 1)
                    try:
                        weights[fid.strip()] = clamp01(float(wstr.strip()))
                    except ValueError:
                        pass
    else:
        #  JSON body
    ...
    #  Add facts that are true in dataset
    if use_true_facts:
        observed_ids.update([fid for fid, val in FACT_VALUES.items() if val is True])

    # Build observation confidence mapping
    obs_conf = {}
...
def facts_new():
    if request.method == "POST":
        ...
        global FACT_VALUES
        FACT_VALUES = {f["id"]: bool(f.get("value", False)) for f in facts}
        flash(f"Fact '{fid}' added successfully.", "success")
        return redirect(url_for("facts_list"))
...
def facts_edit(fid):
    ...
    if request.method == "POST":
        ...
        updated = {"id": fid, "description": description, "value": bool(value), "tags": tags}
        new_facts = [f if f.get("id") != fid else updated for f in facts]
        try:
            validate(new_facts, facts_array_schema)
            write_json("facts.json", new_facts)
        ...
        global FACT_VALUES
        FACT_VALUES = {f["id"]: bool(f["value"]) for f in facts}
        flash(f"Fact '{fid}' updated successfully.", "success")
        return redirect(url_for("facts_list"))
...
def facts_delete(fid):
    ...
    global FACT_VALUES
    FACT_VALUES = {f["id"]: bool(f["value"]) for f in facts}
    flash(f"Fact '{fid}' deleted successfully.", "success")
    return redirect(url_for("facts_list"))
...
def rules_new():
    ...
    return render_template("rule_form.html", mode="new", rules=None)
